...
link.compare.n1<- function(model,s0=0,ns,nrep,muv = 0,sdv =1,model.args = list(),len.newx=500,init.args=list(init=c(0,0),xi0 =1,r0=1,intervalr=c(0.03,10)),spline.control = list(deg = 3,nknots = 10),lamv=seq(5,100,length.out = 20),bound=3,nb=2)                                                                                                                                                                                                                                                                                                  
{
  ### output ####
  ...
  for(j in 1:nrep)
  {
    ...
    gam.fit<- gam(y0~s(x1),family = binomial(link = 'logit'))
