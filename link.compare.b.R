...
link.compare<- function(model,s0=0,ns,nrep,min.value,max.value,
                        ...
                        spline.control = list(deg = 3,nknots = 10))                                                                                                                                                                                                                                                                                                  
{
  ### output ####
  mse.logit <- mse.probit <- mse.gev <- mse.robit <- mse.splogit <- mse.pspline <-  rep(0,nrep)
  
  #### aic ##### 
  #aic.logit <- aic.probit <- aic.gev <- aic.gev.new <- aic.robit <- aic.splogit <- aic.pspline  <- rep(0,nrep)
  
  #### gradient ####
  grv.n1 <- matrix(0,nrep,3)
  ...
  for(s in 1:nrep)
  {
    ...
    pspline.gcv <- pspline.gcv(y0 = y0,xmat = xmat0,qv=1,monotone = TRUE,nknots = 10,beta0 = c(1,1),lam.interval = c(0,100))
    
    pspline.fit <- psplinelink1(y0 = y0,xmat = xmat0,qv=1,monotone = TRUE,nknots = 10,beta0 = c(1,1),lambda=pspline.gcv)
  ...
  mse.mat <- cbind(mse.logit,mse.probit,mse.robit,
                   mse.gev,mse.splogit,mse.pspline)
